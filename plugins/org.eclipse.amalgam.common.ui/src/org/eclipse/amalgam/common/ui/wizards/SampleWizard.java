...
public class SampleWizard extends Wizard implements INewWizard, IExecutableExtension {
	...
	/*
	 * private void doSelectReveal() { if (selection == null ||
	 * createdProjects==null) return; String viewId =
	 * selection.getAttribute("targetViewId"); //$NON-NLS-1$ if (viewId == null)
	 * return; IWorkbenchWindow window = PlatformUI.getWorkbench()
	 * .getActiveWorkbenchWindow(); if (window == null) return; IWorkbenchPage
	 * page = window.getActivePage(); if (page == null) return; IViewPart view =
	 * page.findView(viewId); if (view == null || !(view instanceof
	 * ISetSelectionTarget)) return; ISetSelectionTarget target =
	 * (ISetSelectionTarget) view; IConfigurationElement[] projects =
	 * selection.getChildren("project"); //$NON-NLS-1$
	 * 
	 * ArrayList items = new ArrayList(); for (int i = 0; i < projects.length;
	 * i++) { String path = projects[i].getAttribute("selectReveal");
	 * //$NON-NLS-1$ if (path == null) continue; IResource resource =
	 * createdProjects[i].findMember(path); if (resource.exists())
	 * items.add(resource); } if (items.size() > 0) target.selectReveal(new
	 * StructuredSelection(items)); }
	 */
	...
	/**
	 * @param selectRevealEnabled
	 *            The selectRevealEnabled to set.
	 * @todo Generated comment
	 */
	...
	/**
	 * @param activitiesEnabled
	 *            The activitiesEnabled to set.
	 * @todo Generated comment
	 */
