...
public class PrepareInstallProfileJob implements IRunnableWithProgress {
    ...
    private final Collection<InstallableComponent> installableConnectors;
    ...
    public void doRun(IProgressMonitor monitor) throws CoreException {
        ...
        try {
            ...
            // this is a pre-emptive check so that we can provide better error
            // handling
                ...
                for (InstallableComponent descriptor : installableConnectors) {
                    ...
                    if (host != null && host.length() > 0 && hostnames.add(host)) {
                        try {
                            ...
                            throw new CoreException(new Status(IStatus.ERROR, DiscoveryUIPlugin.PLUGIN_ID, NLS.bind(Messages.PrepareInstallProfileJob_errorResolvingHostname, descriptor.getName(), host), e));
                        }
                    }
                }
            }
            ...
            final Map<IMetadataRepository, URL> repositoryToURL = new HashMap<IMetadataRepository, URL>();
            {
                ...
                for (URL updateSiteUrl : repositoryURLs) {
                    ...
                    IMetadataRepository repository = ProvisioningUtil.loadMetadataRepository(updateSiteUrl.toURI(), new SubProgressMonitor(monitor, unit));
            ...
            // in each repository. We select installable units by matching both
            // the feature id and the repository; it
            // is possible though unlikely that the same feature id is available
            // from more than one of the selected
            // repositories, and we must ensure that the user gets the one that
            // they asked for.
            ...
            // filter those installable units that have a duplicate in the list
            // with a higher version number.
            // it's possible that some repositories will host multiple versions
            // of a particular feature. we assume
            ...
            // Verify that we found what we were looking for: it's possible that
            // we have connector descriptors
            // that are no longer available on their respective sites. In that
            // case we must inform the user.
            ...
            if (installableUnits.size() < installableConnectors.size()) {
                ...
                if (!installableUnits.isEmpty()) {
                    // instead of aborting here we ask the user if they wish to
                    // proceed anyways
                ...
                if (!proceed) {
                    ...
                    for (InstallableComponent descriptor : installableConnectors) {
                        if (!foundIds.contains(descriptor.getId())) {
                            ...
                            notFoundDescription += NLS.bind(Messages.PrepareInstallProfileJob_notFoundDescriptorDetail, new Object[] { descriptor.getName(), descriptor.getId(),
                                    descriptor.getSiteURL() });
                        }
                    }
                    throw new CoreException(new Status(IStatus.ERROR, DiscoveryUIPlugin.PLUGIN_ID, NLS.bind(Messages.InstallConnectorsJob_connectorsNotAvailable, notFoundDescription), null));
                }
            ...
            MultiStatus status = new MultiStatus(DiscoveryUIPlugin.PLUGIN_ID, 0, Messages.PrepareInstallProfileJob_ok, null);
            ius = installableUnits.toArray(new IInstallableUnit[installableUnits.size()]);
            ProfileChangeRequest profileChangeRequest = InstallAction.computeProfileChangeRequest(ius, profileId, status, new SubProgressMonitor(monitor, installableConnectors.size()));
            if (status.getSeverity() > IStatus.WARNING) {
                throw new CoreException(status);
            }
            if (profileChangeRequest == null) {
                // failed but no indication as to why
                throw new CoreException(new Status(IStatus.ERROR, DiscoveryUIPlugin.PLUGIN_ID, Messages.PrepareInstallProfileJob_computeProfileChangeRequestFailed, null));
            }
            PlannerResolutionOperation operation = new PlannerResolutionOperation(Messages.PrepareInstallProfileJob_calculatingRequirements, profileId, profileChangeRequest, null, status, true);
    ...
    private String computeProfileId() throws CoreException {
        ...
        throw new CoreException(new Status(IStatus.ERROR, DiscoveryUIPlugin.PLUGIN_ID, Messages.InstallConnectorsJob_profileProblem, null));
    }
