 ...
 * that's not possible. Copied from
 * {@link org.eclipse.ui.actions.QuickMenuCreator} 3.5 version to have the
 * implementation that does not require a call to dispose method.
 * 
 * @since 3.3
 */
	...
	protected Point computeMenuLocation(Tree tree) {
		...
		case 1:
			...
			if (intersect != null && intersect.height == bounds.height) {
				return new Point(Math.max(0, bounds.x + getAvarageCharWith(tree) * CHAR_INDENT), bounds.y
						+ bounds.height);
			}
			return null;
