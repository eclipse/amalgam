...
import org.eclipse.amalgam.examples.mindmap.diagram.part.MindmapDiagramEditor;
import org.eclipse.amalgam.examples.mindmap.diagram.part.MindmapDiagramEditorPlugin;
import org.eclipse.amalgam.examples.mindmap.diagram.part.MindmapDiagramEditorUtil;
 ...
 * @generated
 */
public class MindmapMetricProvider extends AbstractContributionItemProvider {
	...
	private static class MetricsAction extends Action {
		...
		public void run() {
			...
			try {
				metricsView = PlatformUI
						.getWorkbench()
						.getActiveWorkbenchWindow()
						.getActivePage()
						.findView(
								org.eclipse.amalgam.examples.mindmap.diagram.providers.MindmapMetricProvider.ResultView.VIEW_ID);
				if (metricsView == null) {
					metricsView = PlatformUI
							.getWorkbench()
							.getActiveWorkbenchWindow()
	...
	static void calculateSemanticElementMetrics(
			...
			List/*<ElementMetrics>*/metricsList) {
		Diagram diagram = diagramEditPart.getDiagramView();
		...
		while (next != null) {
			...
			if (MindmapPackage.eINSTANCE.getTopic().isInstance(next)) {
				res.add(new Metric("NOS", calcNOS((Topic) next),
						new Double(0.0), new Double(5.0)));
			}
			if (!res.isEmpty()) {
				ElementMetrics row = new ElementMetrics(next,
						formatElementName(next), (Metric[]) res
								.toArray(new Metric[res.size()]));
				metricsList.add(row);
		...
		if (!target2row.isEmpty()) { // list was modified, need to process only semantic metrics
			...
			for (Iterator it2 = target2row.entrySet().iterator(); it2.hasNext();) {
				...
				View targetView = MindmapDiagramEditorUtil.findView(
	...
	private static String formatViewName(View viewTarget) {
		...
		int visualID = MindmapVisualIDRegistry.getVisualID(viewTarget);
	...
	private static class ElementMetrics {
		...
		ElementMetrics(EObject target, String name, Metric[] metrics) {
			...
			this.elementImage = MindmapElementTypes.getImage(imageTarget);
		}
	...
	private static String[] getMetricToolTips() {
		return new String[] { "Number of Subtopics" + '\n'
				+ "The number of direct subtopics for the selected Topic. "
				+ '\n' + "low: 0.0" + "high: 5.0" };
	}
	 ...
	 * @generated
	 */
	private static MindmapAbstractExpression expressionNOS;

	/**
	 * @generated
	 */
	public static Double calcNOS(Topic target) {
		if (expressionNOS == null) {
			expressionNOS = MindmapOCLFactory.getExpression(
					"self.subtopics->size()", MindmapPackage.eINSTANCE
							.getTopic());
		}
		Object val = expressionNOS.evaluate(target);
	...
	public static class ResultView extends ViewPart implements IOpenListener {
		 ...
		 * @generated
		 */
		public static final String VIEW_ID = "org.eclipse.amalgam.examples.mindmap.diagram.metricView";//$NON-NLS-1$
		...
		public void createPartControl(Composite parent) {
			...
			if (editor != null
					&& editor.getClass().equals(MindmapDiagramEditor.class)) {
				setInput((MindmapDiagramEditor) editor);
			}
		}
				...
				if (viewID != null) {
					...
					if (targetView != null) {
						...
						if (targetEditPart != null) {
							MindmapDiagramEditorUtil.selectElementsInDiagram(
